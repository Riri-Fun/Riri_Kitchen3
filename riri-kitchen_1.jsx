...
export default function RiriKitchen() {
    ...
    (async () => {
      try {
        const [pRes, oRes, aRes, fRes, nRes, mRes, bRes, hRes] = await Promise.all([
          window.storage.get("rk-products", false).catch(() => null),
          window.storage.get("rk-orders", false).catch(() => null),
          window.storage.get("rk-avatar", false).catch(() => null),
          window.storage.get("rk-favorites", false).catch(() => null),
          window.storage.get("rk-notifications", false).catch(() => null),
          window.storage.get("rk-mealprefs", false).catch(() => null),
          window.storage.get("rk-banner", false).catch(() => null),
          window.storage.get("rk-cookhearts", false).catch(() => null),
        ]);
  ...
  async function persistProducts(next) {
    setProducts(next);
    try {
      await window.storage.set("rk-products", JSON.stringify(next), false);
    } catch (e) {}
  }
  ...
  async function persistOrders(next) {
    setOrders(next);
    try {
      await window.storage.set("rk-orders", JSON.stringify(next), false);
    } catch (e) {}
  }
  ...
  function handleAvatarUpload(e) {
    ...
    reader.onload = async () => {
      ...
      try {
        await window.storage.set("rk-avatar", reader.result, false);
      } catch (err) {}
    };
  ...
  async function toggleFavorite(id) {
    ...
    setFavorites(next);
    try {
      await window.storage.set("rk-favorites", JSON.stringify(next), false);
    } catch (e) {}
  }
  ...
  async function publishNotification() {
    ...
    try {
      await window.storage.set("rk-notifications", JSON.stringify(next), false);
    } catch (e) {}
  ...
  async function updateMealPrefs(next) {
    setMealPrefs(next);
    try {
      await window.storage.set("rk-mealprefs", JSON.stringify(next), false);
    } catch (e) {}
  }
  ...
  function handleBannerUpload(e) {
    ...
    reader.onload = async () => {
      ...
      try {
        await window.storage.set("rk-banner", reader.result, false);
      } catch (err) {}
    };
    reader.readAsDataURL(file);
  }
  ...
  async function confirmPayment() {
    ...
    try {
      await window.storage.set("rk-cookhearts", String(nextHearts), false);
    } catch (e) {}
    setCart({});
